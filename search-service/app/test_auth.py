...
def test_search_allows_any_authenticated_user():
    ...
    # Students must have selected/registered courses (mirrors onboarding behavior).
    # Populate the in-memory profile store via /v1/users/me.
    resp = client.post(
        "/v1/users/me",
        json={"role": "student", "courses": ["demo"]},
    )
